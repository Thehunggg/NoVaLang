...
const topicLexicon: Record<LanguageCode, Record<string, [string, string, string, string][]>> = {
  ...
  es: { greeting: [["hola", "hello", "Hola, Maya.", "Hello, Maya."], ["adiós", "goodbye", "Adiós, Leo.", "Goodbye, Leo."], ["gracias", "thank you", "Muchas gracias.", "Thank you very much."], ["por favor", "please", "Agua, por favor.", "Water, please."]], name: [["me llamo", "my name is", "Me llamo Ana.", "My name is Ana."], ["yo soy", "I am", "Yo soy Luis.", "I am Luis."], ["de", "from", "Soy de España.", "I am from Spain."], ["mucho gusto", "nice to meet you", "Mucho gusto.", "Nice to meet you."]], number: [["uno", "one", "Tengo un libro.", "I have one book."], ["cinco", "five", "Veo cinco mesas.", "I see five tables."], ["diez", "ten", "Cuento hasta diez.", "I count to ten."], ["veinte", "twenty", "Hay veinte sillas.", "There are twenty chairs."]], family: [["madre", "mother", "Mi madre es amable.", "My mother is kind."], ["padre", "father", "Mi padre está aquí.", "My father is here."], ["hermana", "sister", "Mi hermana lee.", "My sister reads."], ["hermano", "brother", "Mi hermano sonríe.", "My brother smiles."]], food: [["agua", "water", "Bebo agua.", "I drink water."], ["pan", "bread", "Como pan.", "I eat bread."], ["arroz", "rice", "Me gusta el arroz.", "I like rice."], ["manzana", "apple", "La manzana es roja.", "The apple is red."]], place: [["estación", "station", "La estación está aquí.", "The station is here."], ["banco", "bank", "El banco está allí.", "The bank is there."], ["izquierda", "left", "Gira a la izquierda.", "Turn left."], ["derecha", "right", "Gira a la derecha.", "Turn right."]], time: [["mañana", "morning / tomorrow", "Estudio por la mañana.", "I study in the morning."], ["hoy", "today", "Trabajo hoy.", "I work today."], ["mañana", "tomorrow", "Viajo mañana.", "I travel tomorrow."], ["ayer", "yesterday", "Estudié ayer.", "I studied yesterday."]], default: [["libro", "book", "Esto es un libro.", "This is a book."], ["estudiante", "student", "Soy estudiante.", "I am a student."], ["sí", "yes", "Sí, es correcto.", "Yes, it is correct."], ["pregunta", "question", "Tengo una pregunta.", "I have a question."]] }
};
...
const kanaReadingPairs = (items: ContentItem[]) => unique(items.filter((item): item is PronunciationItem => item.kind === "pronunciation").slice(0, 5).map((item) => ({ left: item.symbol, right: item.pronunciation })), (pair) => pair.left);
...
const makeExercises = (items: ContentItem[], language: LanguageCode, level: LevelId, id: string): Exercise[] => {
  ...
  const words = language === "ja" ? sentence.replace(/[。！？]/g, "").split("") : sentence.replace(/[.!?¡¿]/g, "").split(/\s+/);
  ...
  const localizedMeaningOptions = (locale: "en" | "vi" | "ja" | "es") => unique(items.map((item) => contentMeanings(item)[locale]?.[0] ?? contentMeaning(item)));
  ...
  const readingOptions = pronunciation ? shuffled(unique([firstMeaning, ...jaKana.map((seed) => seed[1]).filter((reading) => reading !== firstMeaning)]).slice(0, 4)) : [];
  const soundOptions = pronunciation ? shuffled(unique([contentMeaning(second), ...items.filter((item): item is PronunciationItem => item.kind === "pronunciation").map((item) => item.pronunciation), ...jaKana.map((seed) => seed[1])]).slice(0, 4)) : shuffled(meanings);
...
const examLevelFor = (language: LanguageCode, levelId: LevelId): ExamLevel | undefined => {
  ...
  if (language === "es") return "GENERAL_SPANISH";
  return undefined;
};
...
const blueprints: Record<LanguageCode, UnitBlueprint[]> = {
  ...
  es: [
    { levelId: "A0", title: "Alphabet and Vowels", goal: "Recognize Spanish letters, vowels, and first words.", lessons: [L("Spanish Alphabet", "pronunciation"), L("Vowels A E I O U", "pronunciation"), L("Basic Pronunciation", "pronunciation"), L("First Words: Hola, Gracias, Sí, No"), L("Unit Review", "review"), L("Checkpoint", "checkpoint")] },
    { levelId: "A0", title: "Pronunciation Patterns", goal: "Recognize common Spanish sound patterns.", lessons: [L("ca / co / cu", "pronunciation"), L("que / qui", "pronunciation"), L("ga / go / gu", "pronunciation"), L("ñ / ll / j / silent h", "pronunciation"), L("Unit Review", "review"), L("Checkpoint", "checkpoint")] },
    { levelId: "A1_1", title: "Greetings and Introductions", goal: "Greet someone and introduce yourself.", lessons: [L("Hola and Adiós"), L("Me llamo..."), L("Soy de..."), L("Mucho gusto", "dialogue"), L("Unit Review", "review"), L("Checkpoint", "checkpoint")] },
    { levelId: "A1_1", title: "Basic Survival Spanish", goal: "Use numbers, classroom words, questions, and polite phrases.", lessons: [L("Numbers 1–20"), L("Classroom Words"), L("Simple Questions", "grammar"), L("Por favor and Gracias", "dialogue"), L("Unit Review", "review"), L("Checkpoint", "checkpoint")] },
    { levelId: "A1_2", title: "People and Things", goal: "Describe family, objects, and possession.", lessons: [L("Family", "vocabulary")] },
    { levelId: "A1_2", title: "Food and Preferences", goal: "Talk about foods and preferences.", lessons: [L("Food and Drinks", "vocabulary")] },
    { levelId: "A2_1", title: "Time and Routine", goal: "Describe time and a normal day.", lessons: [L("Time and Daily Actions", "grammar")] },
    { levelId: "A2_1", title: "Places and Directions", goal: "Ask where places are and follow directions.", lessons: [L("Places in Town", "vocabulary")] },
    { levelId: "A2_2", title: "Shopping and Restaurant", goal: "Manage prices, orders, and requests.", lessons: [L("Shopping and Restaurant Phrases", "dialogue")] },
    { levelId: "A2_2", title: "Past and Future Basics", goal: "Share simple past events and future plans.", lessons: [L("Ayer, Hoy, and Mañana", "grammar")] },
    { levelId: "B1_1", title: "Opinions and Experiences", goal: "Give opinions, explain experiences, and connect longer sentences.", lessons: [L("Opiniones", "grammar"), L("Experiencias Pasadas", "dialogue")] },
    { levelId: "B1_2", title: "Reading and Repair", goal: "Read short texts, explain reasons, and repair conversations.", lessons: [L("Lectura Corta", "vocabulary"), L("Aclarar y Reparar", "dialogue")] },
    { levelId: "B2", title: "Professional Communication", goal: "Handle nuanced expressions, longer reading, and work or academic tasks.", lessons: [L("Solicitudes Profesionales", "dialogue"), L("Lectura Larga", "grammar")] }
  ]
};
...
const specialPronunciationGroups = (language: LanguageCode, lessonTitle: string): { title: string; seeds: PronSeed[] }[] | null => {
  ...
  if (language === "es" && lessonTitle === "Spanish Alphabet") return [["A–E", 0, 5], ["F–J", 5, 10], ["K–Ñ", 10, 15], ["O–T", 15, 21], ["U–Z", 21, 27], ["Alphabet Review", 0, 27]].map(([title, start, end]) => ({ title: String(title), seeds: esAlphabet.slice(Number(start), Number(end)) }));
