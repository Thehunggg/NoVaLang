import { Check, ChevronDown, Globe2, Info, Languages, Search } from "lucide-react";
...
export function NativeLanguageSelector({ compact = false }: { compact?: boolean }) {
  ...
  const isViSuggestion = deviceLanguage?.code === "vi";
  ...
  {showSuggestion && deviceLanguage && <div className="mb-3 rounded-2xl border border-violet-300/25 bg-violet-300/10 p-4"><p className="text-sm font-extrabold text-white">{isViSuggestion ? "Có vẻ bạn đang dùng Tiếng Việt.\nBạn có muốn chọn Tiếng Việt làm ngôn ngữ mẹ đẻ không?" : `It looks like your device language is ${deviceLanguage.name}.\nDo you want to use ${deviceLanguage.name} as your native/UI language?`}</p><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => choose(deviceLanguage)} className="rounded-xl bg-cyan-300 px-4 py-2 text-sm font-black text-cyan-950">{isViSuggestion ? "Chọn Tiếng Việt" : `Use ${deviceLanguage.name}`}</button><button onClick={() => { setShowSuggestion(false); setExpanded(true); }} className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-black text-slate-200">{isViSuggestion ? "Tìm ngôn ngữ khác" : "Search another language"}</button></div></div>}
  <div className="flex items-center gap-3 rounded-2xl border border-cyan-300/25 bg-cyan-300/10 p-4"><span className="grid h-10 w-10 place-items-center rounded-xl bg-cyan-300/15 text-cyan-200"><Globe2 size={20} /></span><span className="min-w-0 flex-1"><span className="block text-[10px] font-black uppercase tracking-wider text-cyan-300">{t("selectedNativeLanguage")}</span><strong className="block truncate text-sm text-white">{selected.nativeName} <span className="font-medium text-slate-400">· {selected.name}</span></strong></span><span className={`rounded-lg px-2 py-1 text-[10px] font-black ${selected.uiSupported ? "bg-emerald-300/10 text-emerald-300" : "bg-amber-300/10 text-amber-300"}`}>{selected.uiSupported ? progress.effectiveUILanguage.toUpperCase() : "EN UI"}</span></div>
