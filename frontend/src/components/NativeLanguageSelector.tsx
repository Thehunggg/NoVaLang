...
export function NativeLanguageSelector({ compact = false }: { compact?: boolean }) {
  ...
  {showSuggestion && deviceLanguage && <div className="mb-3 rounded-2xl border border-violet-300/25 bg-violet-300/10 p-4"><p className="whitespace-pre-line text-sm font-extrabold text-white">{isViSuggestion ? "Có vẻ bạn đang dùng Tiếng Việt.\nBạn có muốn chọn Tiếng Việt làm ngôn ngữ mẹ đẻ không?" : `It looks like your device language is ${deviceLanguage.name}.\nDo you want to use ${deviceLanguage.name} as your native/UI language?`}</p><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => choose(deviceLanguage)} className="rounded-xl bg-gradient-to-r from-cyan-600 to-violet-600 px-4 py-2 text-sm font-black text-white shadow-lg shadow-violet-950/25 hover:from-cyan-500 hover:to-violet-500">{isViSuggestion ? "Chọn Tiếng Việt" : `Use ${deviceLanguage.name}`}</button><button onClick={() => { setShowSuggestion(false); setExpanded(true); }} className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-black text-slate-200">{isViSuggestion ? "Tìm ngôn ngữ khác" : "Search another language"}</button></div></div>}
