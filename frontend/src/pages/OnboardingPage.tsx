...
import { dailyGoalOptions, localizedConfigLabel } from "../data/sharedConfig";
  ...
  return <main className="grid min-h-screen place-items-center bg-[#080510] px-4 py-8 text-white">
    ...
    <div className="relative w-full max-w-3xl">
      ...
      <div className="rounded-[2rem] border border-white/10 bg-white/[.055] p-6 shadow-2xl backdrop-blur-xl sm:p-9">
        ...
        {step === 1 && <section><h1 className="mt-3 font-display text-3xl font-black">{t("chooseLearningLanguage")}</h1><div className="mt-7 grid gap-3 sm:grid-cols-3">{languages.map((item) => <button key={item.code} onClick={() => { setLanguage(item.code); setLevel("A0"); }} className={`relative rounded-2xl border p-5 text-left transition ${language === item.code ? "border-cyan-300/50 bg-cyan-300/10" : "border-white/10 bg-white/[.035]"}`}><span className="text-4xl">{item.flag}</span><strong className="mt-4 block font-display text-lg">{languageLabel(item.code)}</strong><span className="mt-1 block text-xs text-slate-500">{item.nativeName}</span>{language === item.code && <Check className="absolute right-3 top-3 text-cyan-300" size={17} />}</button>)}</div></section>}

        {step === 2 && <section><h1 className="mt-3 font-display text-3xl font-black">{t("chooseGoal")}</h1><div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{dailyGoalOptions.map((item) => <button key={item.minutes} onClick={() => setGoal(item.minutes)} className={`rounded-2xl border p-5 text-left transition ${goal === item.minutes ? "border-fuchsia-300/50 bg-fuchsia-300/10" : "border-white/10 bg-white/[.035]"}`}><Gauge className="text-fuchsia-300" /><strong className="mt-4 block text-lg">{localizedConfigLabel(item.name, progress.effectiveUILanguage)}</strong><span className="mt-1 block text-xs text-slate-400">{item.minutes} {t("minutesDay")}</span></button>)}</div></section>}
        ...
        {step === 4 && manualLevel && <Button variant="ghost" className="mt-5" onClick={() => setManualLevel(false)}><ArrowLeft size={17} />{t("back")}</Button>}
        {step === 4 && !manualLevel && <Button variant="ghost" className="mt-5" onClick={() => setStep(3)}><ArrowLeft size={17} />{t("back")}</Button>}
      </div>
    </div>
  </main>;
}
