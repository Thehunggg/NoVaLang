...
add('Xin chào, tôi là…', 'Hello, I am…', 'こんにちは、〜です');
...
add('Trong tiếng Nhật, cách nói lịch sự và thân mật không phải lúc nào cũng có quan hệ một-một.', 'In Japanese, polite and casual expressions do not always correspond one-to-one.', '日本語では、丁寧な表現とカジュアルな表現が常に一対一で対応するとは限りません。');
...
add('Gặp người chưa thân.', 'When meeting someone you do not know well.', 'まだ親しくない人に会うとき。');
...
add('Người mới gặp', 'People you have just met', '初対面の人');
add('Đồng nghiệp', 'Coworkers', '同僚');
add('Khách hàng', 'Customers', '顧客');
add('Không dùng như lời chào buổi chiều.', 'Do not use it as an afternoon greeting.', '午後のあいさつとしては使いません。');
add('Không dùng như lời chào buổi tối.', 'Do not use it as an evening greeting.', '夜のあいさつとしては使いません。');
add('Không dùng trước khi đi ngủ.', 'Do not use it before going to sleep.', '寝る前には使いません。');
...
add('Không nên dùng おはよう trống với cấp trên hoặc khách hàng khi quan hệ chưa đủ gần.', 'Do not use bare おはよう with supervisors or customers unless the relationship is close enough.', '十分に親しい関係でない上司や顧客には、「おはよう」だけを使わない方が安全です。');
...
add('Gặp người quen không quá thân.', 'When meeting an acquaintance you are not very close to.', 'あまり親しくない知人に会うとき。');
add('Không ghi mốc giờ cứng như 11:00, 12:00 hoặc 17:00.', 'Do not treat times such as 11:00, 12:00, or 17:00 as fixed boundaries.', '11時、12時、17時などを固定的な境目として覚えないでください。');
...
add('Người quen không quá thân', 'Acquaintances you do not know well', 'あまり親しくない知人');
add('Bạn bè trong một số tình huống', 'Friends in some situations', '場面によっては友達');
add('Không dùng thay lời chào buổi sáng khi おはようございます tự nhiên hơn.', 'Do not use it instead of a morning greeting when おはようございます is more natural.', '「おはようございます」の方が自然な朝の場面では、代わりに使いません。');
add('Không dùng sau khi chuyển sang buổi tối khi こんばんは tự nhiên hơn.', 'Do not use it once evening has begun and こんばんは is more natural.', '夜になり「こんばんは」の方が自然な場面では使いません。');
add('Không dùng như lời tạm biệt.', 'Do not use it as a farewell.', '別れのあいさつとしては使いません。');
...
add('Chúng không phải bản thay thế một-một của こんにちは.', 'They are not one-to-one replacements for こんにちは.', '「こんにちは」と一対一で置き換えられる表現ではありません。');
...
add('Thường dùng sau khi trời đã tối.', 'It is commonly used after it gets dark.', '一般に、暗くなってから使います。');
add('Người quen', 'Acquaintances', '知人');
add('Không dùng để nói “chúc ngủ ngon”.', 'Do not use it to say “good night.”', '「おやすみなさい」の意味では使いません。');
add('Không dùng khi chia tay.', 'Do not use it when saying goodbye.', '別れるときには使いません。');
add('Không dùng vào buổi sáng.', 'Do not use it in the morning.', '朝には使いません。');
add('Không dùng giữa ban ngày.', 'Do not use it during the daytime.', '日中には使いません。');
add('Cách thân mật:', 'Casual usage:', 'カジュアルな言い方：');
add('Không có một dạng thân mật chuẩn duy nhất.', 'There is no single standard casual form.', '標準的なカジュアル形は一つに決まっていません。');
...
add('Chào buổi tối. Hôm nay lạnh nhỉ.', 'Good evening. It is cold today, is it not?', 'こんばんは。今日は寒いですね。');
...
add('Không dùng mỗi ngày với cùng một người.', 'Do not use it every day with the same person.', '同じ相手に毎日使う表現ではありません。');
add('Không dùng từ lần gặp thứ hai trở đi.', 'Do not use it from the second meeting onward.', '二回目以降に会うときは使いません。');
add('Không dùng chỉ vì nhìn thấy một người lạ nhưng không làm quen.', 'Do not use it merely because you see a stranger without introducing yourself.', '知らない人を見かけただけで、自己紹介をしない場合には使いません。');
add('Không dùng thay lời chào buổi sáng, ban ngày hoặc buổi tối.', 'Do not use it instead of a morning, daytime, or evening greeting.', '朝・昼・夜のあいさつの代わりには使いません。');
add('Trung tính và lịch sự trong lần đầu gặp.', 'Neutral and polite for a first meeting.', '初対面で使える中立的で丁寧な表現。');
add('Không có dạng thân mật chuẩn bắt buộc.', 'There is no mandatory standard casual form.', '必ず使う標準的なカジュアル形はありません。');
add('Ngay cả hai người cùng tuổi gặp lần đầu vẫn có thể dùng:', 'Even two people of the same age can use:', '同年代同士の初対面でも使えます。');
add('Trong tình huống rất thoải mái có thể bỏ qua cụm này và nói tên trực tiếp, nhưng không được dạy đó là cách thay thế mặc định.', 'In a very relaxed situation, people may omit it and say their name directly, but this is not the default replacement.', 'とてもくだけた場面では省略して名前から言うこともありますが、標準的な代わりとして教えてはいけません。');
add('Rất vui được gặp bạn lần đầu. Tôi là Tanaka.', 'Nice to meet you. I am Tanaka.', 'はじめまして。田中です。');

add('Không cần gọi “Sato-san”, gọi Sato thôi được chứ?', 'You do not need to call me “Sato-san”; is just “Sato” okay?', '「佐藤さん」ではなく、「佐藤」だけでいいですか。');
...
add('Rất vui được gặp bạn. Tôi là Sato. Bạn tên là gì?', 'Nice to meet you. I am Sato. What is your name?', 'はじめまして。佐藤です。お名前は何ですか。');
add('Tanaka, hôm nay cũng cùng cố gắng nhé.', 'Tanaka, let us do our best again today.', '田中さん、今日も一緒に頑張りましょう。');
add('Vâng, mong thầy/cô giúp đỡ.', 'Yes, I look forward to learning from you.', 'はい、よろしくお願いします。');
add('Xin chào. Rất vui được gặp bạn lần đầu. Mình là Tanaka.', 'Hello. Nice to meet you. I am Tanaka.', 'こんにちは。はじめまして。田中です。');
add('Rất vui được gặp bạn lần đầu. Mình là Sato.', 'Nice to meet you. I am Sato.', 'はじめまして。佐藤です。');
...
add('Nói lại nhé, mình là Tanaka. Làm quen nhé!', 'Let me introduce myself again: I am Tanaka. Nice to meet you!', 'あらためて、田中。よろしく！');
add('Mình là Sato. Làm quen nhé!', 'I am Sato. Nice to meet you!', '佐藤。よろしく！');
add('Ừ, được. Tanaka cũng vậy nhé!', 'Yes, that is fine. Same for you, Tanaka!', 'うん、いいよ。田中もよろしく！');
...
add('Trong bài này: Tôi là Tanaka.', 'In this lesson: I am Tanaka.', 'このレッスンでは「田中です」という意味です。');
...
add('Không được dạy ～だよ như bản thay thế bắt buộc của ～です。', 'Do not teach ～だよ as a required replacement for ～です。', '「～だよ」を「～です」の必須の置き換えとして教えてはいけません。');
add('Với giáo viên, người lớn hơn hoặc người mới gặp, dùng ～です。', 'Use ～です with teachers, older people, or people you have just met.', '先生、年上の人、初対面の人には「～です」を使います。');
add('Tôi là Tanaka.', 'I am Tanaka.', '田中です。');
add('Tôi là Sato.', 'I am Sato.', '佐藤です。');
add('Không dịch máy móc câu này trong mọi trường hợp thành “Nice to meet you”.\n\nNghĩa thay đổi theo ngữ cảnh.', 'Do not mechanically translate this expression as “Nice to meet you” in every situation.\n\nIts meaning changes with context.', 'どの場面でも機械的に「Nice to meet you」と訳してはいけません。\n\n意味は文脈によって変わります。');
...
add('Rất vui được gặp bạn lần đầu. Tôi là Tanaka. Rất mong được làm quen.', 'Nice to meet you. I am Tanaka. I look forward to getting to know you.', 'はじめまして。田中です。よろしくお願いします。');
add('Tớ là Tanaka. Làm quen nhé!', 'I am Tanaka. Nice to meet you!', '田中。よろしく！');
...
add('Khi dự kiến một khoảng thời gian mới gặp lại.', 'When you do not expect to meet again for some time.', 'しばらく会わないと予想されるとき。');
add('Không dùng khi vừa gặp.', 'Do not use it when you have just met.', '会ったばかりのときには使いません。');
add('Không dùng thay こんにちは。', 'Do not use it instead of こんにちは。', '「こんにちは」の代わりには使いません。');
add('Không dùng thay lời tự giới thiệu.', 'Do not use it instead of a self-introduction.', '自己紹介の代わりには使いません。');
...
add('またね！\n→ Gặp lại nhé!', 'またね！\n→ See you!', 'またね！\n→ また会おうね！');
add('Không mở rộng chủ đề chia tay quá sâu trong Lesson 1.', 'Do not explore parting expressions in depth in Lesson 1.', 'レッスン1では、別れの表現を深く扱いすぎません。');
...
add('Dùng おはようございます vì đây là buổi sáng.', 'Use おはようございます because it is morning.', '朝なので「おはようございます」を使います。');
add('Học sinh nói với giáo viên nên không dùng おはよう！', 'The student is speaking to a teacher, so おはよう！ is not appropriate.', '生徒が先生に話すため、「おはよう！」は使いません。');
add('よろしくお願いします ở đây thể hiện thiện chí học tập và hợp tác, không chỉ dùng khi mới gặp.', 'Here, よろしくお願いします expresses willingness to learn and cooperate; it is not limited to first meetings.', 'ここでの「よろしくお願いします」は、学習や協力への前向きな気持ちを表し、初対面だけに使う表現ではありません。');
...
add('Hai người cùng tuổi nhưng mới gặp nên dùng cách lịch sự cơ bản.', 'They are the same age, but have just met, so they use standard polite speech.', '同年代でも初対面なので、基本的な丁寧表現を使います。');
add('Không cần chuyển ngay sang cách quá thân mật.', 'There is no need to switch immediately to overly casual speech.', 'すぐにくだけすぎた話し方へ変える必要はありません。');
...
add('Không phải cứ gặp lần đầu là dùng ngay よろしく！', 'Do not use よろしく！ immediately in every first meeting.', '初対面だからといって、すぐに「よろしく！」を使うわけではありません。');
...
add('Tôi là [tên].', 'I am [name].', '私は［名前］です。');
add('Có thể nói:\n私は田中です。\n\nNhưng khi bối cảnh đã rõ là đang tự giới thiệu, tiếng Nhật thường bỏ chủ ngữ:\n田中です。', 'You can say:\n私は田中です。\n\nHowever, when it is clear that you are introducing yourself, Japanese often omits the subject:\n田中です。', '次のように言えます：\n私は田中です。\n\nただし、自己紹介だと明らかな場合、日本語では主語を省くことがよくあります：\n田中です。');
...
add('Nói với người mới gặp', 'Speaking with someone you have just met', '初対面の人に話すとき');
add('Môi trường học tập hoặc công việc', 'In study or work settings', '学習や仕事の場面');
add('Không được giải thích rằng です luôn đổi thành だよ。\n\nだよ có thêm sắc thái thông báo hoặc nhấn mạnh.', 'Do not explain that です always changes to だよ。\n\nだよ adds a nuance of informing or emphasis.', '「です」が常に「だよ」に変わると説明してはいけません。\n\n「だよ」には、伝達や強調のニュアンスが加わります。');
add('Rất vui được gặp bạn lần đầu. Tôi là Sato.', 'Nice to meet you. I am Sato.', 'はじめまして。佐藤です。');
...
add('よろしく\n→ Thân mật', 'よろしく\n→ Casual', 'よろしく\n→ カジュアル');
add('Không được dạy các mức này như có thể thay thế tự do mà không xét quan hệ và tình huống.', 'Do not teach these levels as freely interchangeable without considering the relationship and situation.', '関係や状況を考えずに、これらを自由に置き換えられるものとして教えてはいけません。');
...
add('Không phải lời chào khi vừa gặp.', 'It is not a greeting used when you have just met.', '会ったときのあいさつではありません。');
add('Gặp lại nhé.', 'See you.', 'またね。');
...
add('Câu 1–10', 'Questions 1–10', '第1問～第10問');
...
add('Câu 11–14', 'Questions 11–14', '第11問～第14問');
...
add('Sắp xếp các thẻ thành câu “Tôi là Tanaka.”', 'Arrange the cards to form the sentence “I am Tanaka.”', 'カードを並べて「私は田中です」という文を作ってください。');
...
add('Audio gồm: おはようございます。 Chào buổi sáng. 私（わたし）は田中（たなか）です。 Tôi là Tanaka.', 'The audio says: おはようございます。 Good morning. 私（わたし）は田中（たなか）です。 I am Tanaka.', '音声の内容は「おはようございます。私（わたし）は田中（たなか）です。」です。');
...
add('さん thường được đặt sau tên người khác. Ví dụ: 佐藤（さとう）さん. Thông thường không tự thêm さん vào tên của chính mình khi giới thiệu.', 'さん is usually placed after another person’s name. For example: 佐藤（さとう）さん. You normally do not add さん to your own name when introducing yourself.', '「さん」は通常、相手の名前の後に付けます。例：佐藤（さとう）さん。自己紹介では、通常、自分の名前に「さん」を付けません。');
...
add('Ô đầu tiên chưa phù hợp với thời điểm của cuộc trò chuyện.', 'The first blank does not fit the time of the conversation.', '最初の空欄は、会話の時間帯に合っていません。');
...
add('Câu kết của đoạn làm quen chưa chính xác.', 'The closing expression in the introduction is not correct.', '初対面の会話の結びが正しくありません。');
...
add('Hoàn thành lời tự giới thiệu. Chọn từng ô trống, rồi chọn thẻ phù hợp. Có một thẻ không cần dùng.', 'Complete the self-introduction. Select each blank, then choose the appropriate card. One card is not needed.', '自己紹介を完成させてください。各空欄を選び、適切なカードを選んでください。使わないカードが一枚あります。');
add('こんばんは là lời chào buổi tối, nhưng không cần thiết trong lời tự giới thiệu được yêu cầu ở câu này. Câu hoàn chỉnh gồm lời chào khi gặp lần đầu, tên người nói và câu kết lịch sự.', 'こんばんは is an evening greeting, but it is not needed in the self-introduction requested here. The complete answer includes a first-meeting greeting, the speaker’s name, and a polite closing.', '「こんばんは」は夜のあいさつですが、この問題で求める自己紹介には不要です。完成した文には、初対面のあいさつ、話者の名前、丁寧な結びが含まれます。');
...
add(
  ...
  'One evening, exchange student Tanaka cannot use their phone and does not know the way to Sakura Dormitory. Tanaka meets Sato and asks for directions. Sato guides Tanaka there.',
...
add('Không dùng よろしく！ với:', 'Do not use よろしく！ with:', 'よろしく！を使わない相手：');
add('Cấp trên', 'Supervisors', '上司');
add(
  'Người mới gặp trong hoàn cảnh trang trọng',
  'People you have just met in a formal setting',
  '改まった場面で初めて会う人',
);
add(
