...
// ── Dialogue groups ──────────────────────────────────────────────────────
add('Vai vế: hai bạn học cùng lớp, chưa thân — giữ thể lịch sự. Bối cảnh: 佐藤 làm rơi cục tẩy; 伊藤 nhận thấy và đề nghị cùng tìm, rồi tìm thấy.', 'Relationship: two classmates who are not yet close — keeping the polite register. Situation: Sato dropped an eraser; Ito notices and offers to help look, then finds it.', '関係性：まだ親しくないクラスメート同士 — 丁寧体を保っています。場面：佐藤が消しゴムを落とし、伊藤が気づいて一緒に探そうと言い、見つけます。');
add('Nếu vậy thì chắc nó ở gần đây thôi. Để tôi tìm giúp.', "Then it should be nearby. Let me help look.", 'それなら、近くにあるはずですね。私も探します。');
...
add('Vai vế: như đã thiết lập ở bài trước (m02-u1-l1) — hai người ngang vai trong chuyến du lịch, giữ thể lịch sự. Bối cảnh: nối tiếp NGAY từ lượt nhờ chụp ảnh đã học ở bài trước — 田中 nhờ 佐藤 chụp giúp, 佐藤 nhận lời và chụp xong, được cảm ơn, và LẦN NÀY đáp lại どういたしまして (câu đáp còn thiếu ở bài trước).', 'Relationship: as already established in the previous lesson (m02-u1-l1) — two people of equal standing on a trip, keeping the polite register. Situation: this continues RIGHT FROM the turn where Tanaka asks for the photo, already taught in the previous lesson — Tanaka asks Sato to take the photo, Sato agrees and finishes taking it, is thanked, and THIS TIME replies with どういたしまして (the reply that lesson left out).', '関係性：前のレッスン（m02-u1-l1）ですでに設定した通り — 旅行中の対等な二人、丁寧体を保っています。場面：前のレッスンで習った、田中が写真をお願いするところから続きます — 田中が佐藤に撮影を頼み、佐藤が引き受けて撮り終え、お礼を言われ、今回は「どういたしまして」と返事をします（前のレッスンでは抜けていた返事）。');
...
add('Đây là đoạn nối tiếp bài trước — 5 lượt đầu (ハチ公と一緒に写真を撮ってもらえませんか／シャッターはどこですか／右上にあります／では撮りますよ／ありがとうございました) đã học ở m02-u1-l1, câu どういたしまして là câu MỚI của bài này.', 'This continues the previous lesson — the first turns (ハチ公と一緒に写真を撮ってもらえませんか／シャッターはどこですか／右上にあります／では撮りますよ／ありがとうございました) were already taught in m02-u1-l1; どういたしまして is new in this lesson.', 'これは前のレッスンの続きです — 最初のやり取り（ハチ公と一緒に写真を撮ってもらえませんか／シャッターはどこですか／右上にあります／では撮りますよ／ありがとうございました）はm02-u1-l1ですでに習っており、「どういたしまして」がこのレッスンの新しい部分です。');
...
// ── vocabularyReferences bổ sung (§G7 vùng B điều kiện 3, owner 2026-08-02) ──
add('Tìm thấy.', 'Found it.', '見つける。');
add('À, tìm thấy rồi.', 'Ah, found it.', 'あ、見つけました。');
add('Dưới chân, ngay chỗ mình đứng.', 'At one’s feet, right where one is standing.', '足元。');
add('Nó ở dưới chân tôi này.', 'It was right at my feet.', '私の足元にありましたよ。');
add('Nút chụp (máy ảnh).', 'The shutter button (camera).', 'シャッター。');
add('Nút chụp ở đâu vậy?', 'Where is the shutter button?', 'シャッターはどこですか？');
add('Cảm giác, kiểu/dáng vẻ thế này.', 'A feeling, a look/style like this.', '感じ。');
add('Kiểu này được không?', 'Is this look okay?', 'このような感じでどうですか？');
add('Sự giúp đỡ, chăm sóc — thường dùng trong お世話になっています (được giúp đỡ/mang ơn).', 'Help, care — usually used in お世話になっています (being helped/indebted).', 'お世話。');
add('Tượng chú chó Hachiko, nổi tiếng ở nhà ga Shibuya.', 'The Hachiko dog statue, famous at Shibuya station.', 'ハチ公。');
add('Có thể nhờ ai làm gì cho mình (thể khả năng của もらう — nhận được).', 'To be able to have someone do something for you (potential form of もらう — to receive).', 'もらえる。');
add('Anh/chị chụp giúp tôi tấm ảnh được không?', 'Could you take a photo for me?', '写真を撮ってもらえませんか？');
add('Luôn được anh/chị giúp đỡ nhiều.', 'I am always helped by you a lot.', 'いつもお世話になっています。');
add('Nếu vậy thì, trong trường hợp đó.', 'In that case, if so.', 'それなら。');
add('Nếu vậy thì chắc nó ở gần đây thôi.', 'Then it should be nearby.', 'それなら、近くにあるはずですね。');
add('Chắc nó ở gần đây thôi.', 'It should be nearby.', '近くにあるはずですね。');
add('Hẳn là, chắc là (dự đoán có căn cứ).', 'Should be, is likely to be (a grounded expectation).', 'はず。');
add('Tìm kiếm.', 'To search, to look for.', '探す。');
add('Nếu vậy thì.', 'In that case.', 'それなら。');
add('Cảm giác, kiểu/dáng vẻ.', 'A feeling, a look/style.', '感じ。');
add('Sự giúp đỡ, chăm sóc.', 'Help, care.', 'お世話。');
add('Tôi cũng đi tìm giúp.', "I'll help look too.", '私も探します。');
