...
// ── Dialogue groups ──────────────────────────────────────────────────────
add('Vai vế: hai bạn học cùng lớp, chưa thân — giữ thể lịch sự theo đúng trạng thái trong nguồn. Bối cảnh: 佐藤 làm rơi cục tẩy, 伊藤 giúp tìm và tìm thấy.', 'Relationship: two classmates who are not yet close — keeping the polite register as it appears in the source. Situation: Sato dropped an eraser, and Ito helps look and finds it.', '関係性：まだ親しくないクラスメート同士 — 出典どおり丁寧体を保っています。場面：佐藤が消しゴムを落とし、伊藤が捜すのを手伝って見つけます。');
...
add('Vai vế: như đã thiết lập ở bài trước (m02-u1-l1) — hai người ngang vai trong chuyến du lịch, giữ thể lịch sự. Bối cảnh: nối tiếp NGAY SAU đoạn đã học ở bài trước — 佐藤 vừa chụp ảnh xong cho 田中, được cảm ơn, và LẦN NÀY đáp lại どういたしまして (câu đáp còn thiếu ở bài trước).', 'Relationship: as already established in the previous lesson (m02-u1-l1) — two people of equal standing on a trip, keeping the polite register. Situation: this continues RIGHT AFTER the passage already taught in the previous lesson — Sato has just finished taking a photo for Tanaka, is thanked, and THIS TIME replies with どういたしまして (the reply that lesson left out).', '関係性：前のレッスン（m02-u1-l1）ですでに設定した通り — 旅行中の対等な二人、丁寧体を保っています。場面：前のレッスンで習った箇所のすぐ続き — 佐藤が田中の写真を撮り終えてお礼を言われ、今回は「どういたしまして」と返事をします（前のレッスンでは抜けていた返事）。');
...
add('Đây là đoạn nối tiếp bài trước — 4 lượt đầu (右上にあります／では撮りますよ／ありがとうございました) đã học ở m02-u1-l1, câu どういたしまして là câu MỚI của bài này.', 'This continues the previous lesson — the first turns (右上にあります／では撮りますよ／ありがとうございました) were already taught in m02-u1-l1; どういたしまして is new in this lesson.', 'これは前のレッスンの続きです — 最初のやり取り（右上にあります／では撮りますよ／ありがとうございました）はm02-u1-l1ですでに習っており、「どういたしまして」がこのレッスンの新しい部分です。');
...
add('Theo 敬語の指針: とんでもない vốn là MỘT tính từ trọn vẹn, nên とんでもございません (đổi phần ない) tuy phổ biến nhưng bị coi là chưa chuẩn; dạng CHUẨN là とんでもないです／とんでもないことでございます.', 'Per the 敬語の指針 (Japanese honorifics guidance): とんでもない is originally ONE whole adjective, so とんでもございません (replacing just the ない part) is widely used but considered not strictly standard; the STANDARD forms are とんでもないです／とんでもないことでございます.', '敬語の指針によると、「とんでもない」はもともと一語の形容詞なので、「ない」の部分だけを変えた「とんでもございません」は広く使われているものの、厳密には標準的ではないとされます。標準的な形は「とんでもないです」「とんでもないことでございます」です。');
add('Trấn an người khác đừng bận tâm/lo lắng, lịch sự.', 'Reassure someone not to worry, politely.', '相手に気にしないでほしいと、丁寧に伝える。');
add('Luôn đi kèm ください trong nguồn tìm được — chưa tìm thấy dạng đứng riêng.', 'Always appears together with ください in the sources found — no standalone form has been found.', '見つかった出典では常に「ください」とセットで使われており、単独の形は見つかっていません。');
...
add('Cả hai giữ thể lịch sự vì là bạn học chưa thân, đúng trạng thái trong nguồn.', 'Both keep the polite register since they are classmates who are not yet close, matching the source.', '二人ともまだ親しくないクラスメートなので丁寧体を保っています。出典どおりです。');
...
add('Theo JMdict, とんでもない cũng có dạng kanji hiếm とんでも無い／飛んでもない, nhưng thực tế luôn viết bằng hiragana.', 'Per JMdict, とんでもない also has a rare kanji form とんでも無い／飛んでもない, but it is always written in hiragana in practice.', 'JMdictによると、「とんでもない」には稀な漢字表記「とんでも無い」「飛んでもない」もありますが、実際には常にひらがなで書かれます。');
