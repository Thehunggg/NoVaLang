...
3. **Đo hội thoại ở MỨC KHỐI**, dùng `node scripts/estimate-source-coverage.mjs` (hàm `uniqueUnknownInSpan`/`bestQualifyingSpan`, xây ở lượt build m02-u1-l1) — đếm từ lạ **DUY NHẤT trên toàn khối** (union, không cộng dồn theo từng lượt riêng). **Ngưỡng làm việc ≤6 từ lạ/khối** (G14-R5, owner chốt 2026-08-02, nâng lần 2 trong ngày từ ≤4 — sau lượt nới ≤2→≤4, preview `m02-u1-l2` vẫn cụt ở khối cần thêm nhiều từ lạ hơn ≤4 mới đạt độ dài mong muốn). Báo cáo PHA A phải in **cả bốn mức ≤2/≤3/≤4/≤6** cho mỗi khối để owner thấy đánh đổi. Mọi từ lạ dùng ở ngưỡng >2 **bắt buộc** có mục nghĩa trong `vocabularyReferences` (§G7 vùng B điều kiện 3, §B2f) — thiếu mục nghĩa thì khối đó KHÔNG được dùng dù đủ ngưỡng số lượng.
   - **TRẦN TOÀN BÀI (cổng CỨNG, owner chốt 2026-08-02):** tổng từ lạ DUY NHẤT của CẢ BÀI (union mọi khối, khử trùng lặp giữa các khối) phải **≤ 1,5 × `vocabulary[].length`** của bài. Báo cáo PHA A phải tính và in số này (tổng đo được / trần) — vượt trần thì KHÔNG được nới hết `≤6` cho mọi khối; phải cân đối lại NGAY TRONG PHA A (chọn khối nào nới, khối nào giữ ngắn) trước khi sang PHA B, không để PHA B mới phát hiện.
   - **Đoạn liền kề dùng nhân vật ngoài roster không còn là lý do dừng nới** — xem §G10a: được đổi tên nhân vật sang roster đã duyệt để lấy đoạn dài hơn, miễn nhất quán trong khối và hợp vai vế.
...
1. **Cấu hình A** theo G14-R3b (đọc lại rule tại thời điểm chạy — không chép số cũ, rule có thể đã cập nhật). Với mỗi `dialogueGroup`: lấy đoạn **DÀI NHẤT** mà nguồn cho phép trong giới hạn lọc R5 mức khối (đã đo ở PHA A bước 3), không cắt ngắn xuống vừa đủ ngưỡng dưới nếu nguồn cho phép dài hơn. Đoạn liền kề dùng tên nhân vật ngoài roster → **đổi tên theo §G10a** (chỉ đổi tên người, nhất quán trong khối, hợp vai vế) thay vì bỏ qua đoạn đó. Sau khi nới xong: **kiểm lại TRẦN TOÀN BÀI** (PHA A bước 3) — vượt trần thì rút ngắn khối dài nhất, ghi rõ đã rút khối nào từ mấy xuống mấy lượt.
...
4b. **§G7 vùng B / G14-R5** — mọi từ lạ dùng trong `dialogueGroups` (hội thoại card 3) **BẮT BUỘC** có mục nghĩa trong `vocabularyReferences` (`term`/`reading`/`speechText`/`meaning`/`register`/`example`, đúng hình dạng đã dùng cho `半年`/`なんとか` ở `m01-u2-l2`) trước khi commit. Thiếu mục nghĩa cho dù chỉ một từ lạ → **không dùng khối đó** ở độ dài hiện tại, lùi về đoạn ngắn hơn hoặc thêm mục nghĩa trước, không commit thiếu. `verify-provenance.mjs` gác bằng máy — chạy TRƯỚC khi coi PHA B xong, không phải sau khi phát hiện FAIL mới quay lại vá.
...
5. **Phạm vi ghi file** — chỉ được tạo/sửa trong:
   ...
   - `scripts/validate-curriculum.mjs` — **CHỈ** dòng đăng ký lessonId mới vào
     `APPROVED_READY_DAILY_LESSON_IDS` **hoặc registry tương đương** (một
     dòng `"<lessonId>",` nối vào Set/danh sách đăng ký có sẵn, giống cách
     `FIVE_CARDS_REGISTRY` đăng ký trong `helpers.mjs`). Đây là bước đăng ký
     bắt buộc để bài được coi là ready/playable — không có dòng này thì
     `validate:curriculum` tự chặn bài mới. **Không đụng bất kỳ chỗ nào
     khác** trong file (owner chốt 2026-08-02/2026-08-05, sau khi xác nhận
     đây là bước đăng ký hợp lệ ở bài m02-u2-l1/l2).
